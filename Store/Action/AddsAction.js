...
export const addThumbNailImage = (
  ...
  for (let key in GImage) {
    GelleryImage.push({
      ...
      data: RNFetchBlob.wrap(GImage[key].path),
    });
  }
  ...
  for (let key in FImages) {
    FloorImages.push({
      ...
      data: RNFetchBlob.wrap(FImages[key].path),
    });
  }
  ...
  for (let key in InspectionImagess) {
    InspectImages.push({
      ...
      type: InspectionImagess[key].mime,
       ...
       data: RNFetchBlob.wrap(InspectionImagess[key].path),
    });
  }
    ...
    ? [
        {
          ...
          data: RNFetchBlob.wrap(thumbNailImg.path),
        },
      ]
    : [];
    ...
    ? [
        {
          ...
          data: RNFetchBlob.wrap(pickVedio.path),
        },
      ]
    : [];
...
export const postCarImages = (token, pickImage) => {
  return async dispatch => {
    ...
    //     let response = await RNFetchBlob.fetch(
    //         'POST',
    //         `https://192.168.0.109:45458//api/v1/vendor/cars/108/Images`,
    ...
    //formdata.append("gallery", fileInput.files[0], "/C:/Faizan/Technomites/Projects/On My Way/Images/Rectangle 737/Rectangle 598.png");
    ...
    // formdata.append("document", fileInput.files[0], "/path/to/file");
    ...
    await fetch(
      'http://nowbuysell.dwtdemo.com//api/v1/vendor/cars/109/Images',
      requestOptions,
    )
